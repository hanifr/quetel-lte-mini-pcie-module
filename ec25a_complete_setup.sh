...
get_module_info() {
    ...
    local at_port=$(cat /tmp/ec25a_at_port 2>/dev/null || echo "3")
    echo -e "\n${BLUE}--- AT Command Information ---${NC}"
...
check_signal() {
    ...
    local at_port=$(cat /tmp/ec25a_at_port 2>/dev/null || echo "3")
    echo -e "\n${BLUE}--- AT Command Signal Check ---${NC}"
    ...
    if echo "$signal_response" | grep -q "+CSQ: 99,99"; then
        ...
        echo "   📡 Verify antenna installation:"
        echo "   - UFL connector properly seated"  
        echo "   - Antenna positioned correctly"
        echo "   - Try different location/orientation"
        echo ""
        warning "Antenna might need repositioning or better connection!"
        return 1
        ...
        if [ "$signal_num" -ge 10 ]; then
            success "EXCELLENT signal detected! (+CSQ: $signal_num,xx) - Antenna working perfectly!"
        elif [ "$signal_num" -ge 5 ]; then
            success "Good signal detected! (+CSQ: $signal_num,xx) - Antenna working!"
        else
            warning "Weak signal detected (+CSQ: $signal_num,xx) - Try repositioning antenna"
        fi
...
main() {
    ...
    echo ""
    echo -e "${BLUE}======================================${NC}"
    echo -e "${BLUE}     CRITICAL: ANTENNA CHECK${NC}"
    echo -e "${BLUE}======================================${NC}"
    ...
    if check_signal; then
        ...
        echo ""
        warning "Setup complete, but antenna needs attention!"
        echo ""
        echo "📋 Next steps:"
        echo "1. Check UFL antenna connection is secure"
    ...
    echo ""
    echo "📁 Created files:"
    echo "   ~/ec25a_test_commands.sh - Quick test commands"
